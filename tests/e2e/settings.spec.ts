...
test.describe('Settings - AI Settings', () => {
  ...
  test('should display assignment confidence threshold slider', async ({ window }) => {
    await navigateToSettings(window);

    // Check for Assignment Confidence Threshold label
    const thresholdLabel = window.locator('text=Assignment Confidence Threshold');
    await expect(thresholdLabel).toBeVisible();

    // Check for range slider
    const slider = window.locator('input[type="range"]').filter({
      has: window.locator('xpath=ancestor::div[contains(., "Assignment Confidence Threshold")]')
    }).first();

    await expect(slider).toBeAttached();

    // Check for percentage display
    const percentDisplay = window.locator('div.text-green-400').filter({
      has: window.locator('xpath=ancestor::div[contains(., "Assignment Confidence Threshold")]')
    }).first();
  ...
  test('should adjust assignment confidence threshold', async ({ window }) => {
    await navigateToSettings(window);
    ...
    const percentDisplay = window.locator('text=80%').first();
    await expect(percentDisplay).toBeVisible();
  });

  test('should display account confidence threshold slider', async ({ window }) => {
    await navigateToSettings(window);

    // Check for Account Confidence Threshold label
    const thresholdLabel = window.locator('text=Account Confidence Threshold');
    await expect(thresholdLabel).toBeVisible();

    // Check for range slider
    const slider = window.locator('input[type="range"]').filter({
      has: window.locator('xpath=ancestor::div[contains(., "Account Confidence Threshold")]')
    }).first();

    await expect(slider).toBeAttached();
  });

  test('should adjust account confidence threshold', async ({ window }) => {
    await navigateToSettings(window);
    ...
    // Verify percentage display updated
    const percentDisplay = window.locator('text=90%').first();
    await expect(percentDisplay).toBeVisible();
  });

  test('should enforce min/max values for confidence thresholds', async ({ window }) => {
    await navigateToSettings(window);

    // Test assignment threshold bounds (50-90%)
    const assignmentSlider = window.locator('input[type="range"]').filter({
      has: window.locator('xpath=ancestor::div[contains(., "Assignment Confidence Threshold")]')
    }).first();

    // Get min and max attributes
    const assignmentMin = await assignmentSlider.getAttribute('min');
    const assignmentMax = await assignmentSlider.getAttribute('max');

    expect(assignmentMin).toBe('50');
    expect(assignmentMax).toBe('90');

    // Test account threshold bounds (60-100%)
    const accountSlider = window.locator('input[type="range"]').filter({
      has: window.locator('xpath=ancestor::div[contains(., "Account Confidence Threshold")]')
    }).first();

    const accountMin = await accountSlider.getAttribute('min');
    const accountMax = await accountSlider.getAttribute('max');

    expect(accountMin).toBe('60');
    expect(accountMax).toBe('100');
  });
});
...
test.describe('Settings - Reset Functionality', () => {
  ...
  test('should reset confidence thresholds to defaults', async ({ window }) => {
    await navigateToSettings(window);

    // Change assignment confidence threshold
    const assignmentSlider = window.locator('input[type="range"]').filter({
      has: window.locator('xpath=ancestor::div[contains(., "Assignment Confidence Threshold")]')
    }).first();

    await assignmentSlider.fill('50');
    await window.waitForTimeout(600);

    // Verify it changed
    const changedValue = await assignmentSlider.inputValue();
    ...
    // Verify threshold was reset to default (70%)
    const resetValue = await assignmentSlider.inputValue();
    expect(resetValue).toBe('70');
  });
});
