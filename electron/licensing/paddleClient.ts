...
export class PaddleClient {
     ...
     */
    async verifyLicense(licenseKey: string, deviceId: string): Promise<License> {
        ...
        try {
            ...
            if (!response.ok) {
                throw new LicenseError(
                    LicenseErrorCode.API_ERROR,
                    `Paddle API error: ${response.status} ${response.statusText}`
                );
            }

            const data = await response.json() as PaddleLicenseResponse;

            if (!data.success) {
                throw new LicenseError(
                    LicenseErrorCode.INVALID_LICENSE_KEY,
            ...
            throw new LicenseError(
                ...
                'Failed to verify license with Paddle',
                error
            );
        }
    }
    ...
    async activateDevice(licenseKey: string, deviceFingerprint: DeviceFingerprint): Promise<void> {
        ...
        try {
            ...
            throw new LicenseError(
                LicenseErrorCode.NETWORK_ERROR,
                'Failed to activate device with Paddle',
                error
            );
        }
    }
    ...
    async deactivateDevice(licenseKey: string, deviceId: string): Promise<void> {
        ...
        try {
            ...
            throw new LicenseError(
                LicenseErrorCode.NETWORK_ERROR,
                'Failed to deactivate device with Paddle',
                error
            );
        }
    }
     ...
     */
    private transformPaddleResponse(
        paddleResponse: PaddleLicenseResponse,
        licenseKey: string,
        deviceId: string
    ): License {
        const now = Date.now();
