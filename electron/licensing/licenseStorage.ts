...
import { License, LicenseError, LicenseErrorCode } from './types.js';
...
export class LicenseStorage {
     ...
     */
    static async updateLicense(updates: Partial<License>): Promise<void> {
        try {
     ...
     */
    static async deleteLicense(): Promise<void> {
        try {
            if (fs.existsSync(LICENSE_FILE_PATH)) {
     ...
     */
    static async hasLicense(): Promise<boolean> {
        try {
     ...
     */
    static async backupLicense(backupPath: string): Promise<void> {
        try {
            if (!fs.existsSync(LICENSE_FILE_PATH)) {
                throw new LicenseError(
                    LicenseErrorCode.STORAGE_ERROR,
                    'No license to backup'
                );
            }
     ...
     */
    static async restoreLicense(backupPath: string): Promise<void> {
        try {
     ...
     */
    static validateLicenseData(license: License): boolean {
        try {
     ...
     */
    static async getStorageMetadata(): Promise<{
        exists: boolean;
        path: string;
        size?: number;
        modified?: Date;
