...
export interface AISettings {
    ...
    autoRecordMeetings: boolean;       // Default: true - Auto-record when mic/camera in use
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // Sync auto-record meetings setting to main process when settings are loaded
    useEffect(() => {
        if (migrationComplete && window.electron?.ipcRenderer?.meeting?.setAutoRecordEnabled) {
            const enabled = settings.ai?.autoRecordMeetings ?? true;
            window.electron.ipcRenderer.meeting.setAutoRecordEnabled(enabled);
            console.log('[SettingsContext] Synced autoRecordMeetings to main:', enabled);
        }
    }, [migrationComplete, settings.ai?.autoRecordMeetings]);
