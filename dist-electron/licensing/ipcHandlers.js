...
import { LicenseValidator, LicenseStorage, DeviceFingerprintService, DEFAULT_LICENSE_CONFIG, } from './index.js';
// Global license validator instance
let licenseValidator = null;
/**
 * Initialize licensing system
 */
export function initializeLicensing() {
 ...
 */
async function handleActivateLicense(event, licenseKey, email) {
    try {
 ...
 */
async function handleDeactivateDevice(event, deviceId) {
    try {
        if (!licenseValidator) {
            throw new Error('License validator not initialized');
        }
 ...
 */
async function handleGetTrialInfo() {
    try {
        const license = await LicenseStorage.getLicense();
        if (!license) {
            return {
 ...
 */
async function handleIsLicenseValid() {
    try {
        if (!licenseValidator) {
            throw new Error('License validator not initialized');
        }
 ...
 */
async function handleHasFeature(event, featureName) {
    try {
        const license = await LicenseStorage.getLicense();
        if (!license) {
            return {
        ...
        const hasFeature = license.features[featureName] || false;
        return {
            success: true,
            hasFeature,
        };
 ...
 */
export function getLicenseValidator() {
