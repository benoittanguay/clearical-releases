 ...
 */
import { LicenseStatus, PlanType, ValidationMode, LicenseError, LicenseErrorCode, DEFAULT_LICENSE_CONFIG, LicenseEventType, } from './types.js';
...
export class LicenseValidator {
     ...
     * @returns Validation result
     */
    handleOfflineMode(cachedLicense) {
     ...
     */
    isTrialEndingSoon(license, warningDays) {
     ...
     */
    static getTrialDaysRemaining(license) {
        if (!LicenseValidator.isTrial(license) || !license.trialEndsAt) {
            return 0;
        }
        const timeRemaining = license.trialEndsAt - Date.now();
     ...
     */
    static isInGracePeriod(license) {
     ...
     */
    async startGracePeriod(license) {
        const now = Date.now();
     ...
     */
    static isDeviceLimitReached(license) {
     ...
     * @returns Updated license
     */
    async activateDevice(license, deviceFingerprint) {
        // Check if device already activated
        const existingDevice = license.devices.find((d) => d.deviceId === deviceFingerprint.deviceId);
        if (existingDevice) {
            // Device already activated, just update last seen
            const updatedDevices = license.devices.map((d) => d.deviceId === deviceFingerprint.deviceId
        ...
        if (LicenseValidator.isDeviceLimitReached(license)) {
            throw new LicenseError(LicenseErrorCode.DEVICE_LIMIT_REACHED, `Device limit reached (${license.maxDevices} devices)`);
        }
        // Add new device
     ...
     * @returns Updated license
     */
    async deactivateDevice(license, deviceId) {
        const updatedDevices = license.devices.filter((d) => d.deviceId !== deviceId);
        if (updatedDevices.length === license.devices.length) {
            throw new LicenseError(LicenseErrorCode.VALIDATION_FAILED, 'Device not found in license');
        }
        await LicenseStorage.updateLicense({ devices: updatedDevices });
        this.emitEvent({
